...
function getAverage(value1, value2) {
  const num1 = value1 / 2;
  const num2 = value2 / 2;
  if (Math.abs(num1 + num2) <= Number.MAX_VALUE) return num1 + num2;
  return 0;
}
...
function getDistanceBetweenPoints(x1, y1, x2, y2) {
  const point1 = x1 - x2;
  const point2 = y1 - y2;
  const difference = Math.sqrt(point1 ** 2 + point2 ** 2);
  return difference;
}
...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  ...
  const result = Math.acos(num / denom);
  return result;
}
...
function isPowerOfTwo(num) {
  if (Math.log2(num) % Math.round(Math.log2(num)) === 0) return true;
  return false;
}
